...
ip netns exec mar ip addr add 10.64.11.2/24 dev mar
ip netns exec car ip addr add 10.64.11.3/24 dev car
ip netns exec tar ip addr add 10.64.10.2/24 dev tar
ip netns exec par ip addr add 10.64.10.3/24 dev par

# Remove auto-add routes from network namespaces
ip netns exec mar ip route del 10.64.11.0/24
ip netns exec car ip route del 10.64.11.0/24
ip netns exec tar ip route del 10.64.10.0/24
ip netns exec par ip route del 10.64.10.0/24

# Add default routes to network namespaces
ip netns exec tar ip route add default via 10.64.10.1 dev tar onlink
ip netns exec par ip route add default via 10.64.10.1 dev par onlink
ip netns exec car ip route add default via 10.64.11.1 dev car onlink
ip netns exec mar ip route add default via 10.64.11.1 dev mar onlink
...
iptables -t nat -A PREROUTING -i ens3 -p tcp -m tcp --dport 8080 -j DNAT --to-destination 10.64.11.2:80
