...
# Create veth router <--> host
sudo ip link add host2router type veth peer name router2host
sudo ip link set dev router2host netns router
sudo ip netns exec router ip addr add 10.64.3.2/24 dev router2host
sudo ip netns exec router ip link set dev router2host up
sudo ip addr add 10.64.3.1/24 dev host2router

# Set router default route
sudo ip netns exec router ip route add default via 10.64.3.1
